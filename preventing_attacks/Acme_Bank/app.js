...
// custom message if the csrf token is the cause of an error
...
app.post("/download", body('file').not().isEmpty().trim().escape(), function (request, response) {
  if (request.session.loggedin) {
    ...
    try {
      // on peut aussi utiliser indexOf() and comparing it to less than 0
...
app.post("/public_forum", body('comment').not().isEmpty().trim().escape(), function (request, response) {
  if (request.session.loggedin) {
...
app.get("/public_ledger", function (request, response) {
  if (request.session.loggedin) {
    ...
    if (id) {
      db.all(
        `SELECT * FROM public_ledger WHERE from_account = $id`, {$id: id},
        (err, rows) => {
